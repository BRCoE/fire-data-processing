...
def pack_netcdf(var_netcdf, date, areas_mask, veg_mask, temp_file):

    var_array = var_netcdf.sel(time=date)['decile_values'].data
    ...
    for id in areas_id:
        ...
        stats_dict['avg'].append(np.round(np.nanmean(area_masked_array), decimals=0))
        ...
        for veg_type in [1,2,3]: # 1->grass, 2->shurb, 3->forest
            ...
            stats_dict['avg_{}'.format(veg_type)].append(np.round(np.nanmean(veg_masked_array), decimals=0))
...
def update_netcdf(output_dest, var_netcdf, date, areas_mask, veg_mask, temp_dest_folder):
    
    temp_file = os.path.join(temp_dest_folder, uuid.uuid4().hex + '.nc')

    pack_netcdf(var_netcdf, date, areas_mask, veg_mask, temp_file)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="""create relative zonal stats using shapefiles""")
    parser.add_argument('-decfolder', '--decilefolder', type=str, required=True, help="path to folder with input deciles mosaics")
    ...
    # module load cdo
    # /g/data/xc0/software/conda-envs/rs3/bin/python zonalstats_zonal_stats_relative.py -decfolder /g/data/ub8/au/FMC/stats -vegmaskfolder /g/data/ub8/au/FMC/intermediary_files/vegetation_mask -areafolder /g/data/ub8/au/FMC/intermediary_files/areal_classifications -area both -var both -ystart 2001 -yend 2023 -outfolder /g/data/ub8/au/FMC/stats/zonal_stats -tmpfolder /g/data/ub8/au/FMC/tmp
    ...
    for year in range(y_start, y_end+1):
        ...
        if args.variable == 'both':
            ...
            for var in ['fmc','flam']:
                var_ds = xr.open_dataset('{}/{}_c6_{}_dc.nc'.format(args.decilefolder, var, year))   

                if args.areaclassification == 'both':
                    for area in ['fwa','lga']:
                        areas_mask = np.load('{0}/{1}/{1}.npz'.format(args.areafilefolder, area))['{}'.format(area)]
                        output_path = '{}/{}_{}_zonal_stat.nc'.format(args.outputfolder, area.upper(), var)
                        output_dates = get_stack_dates(output_path)
                        for date in var_ds.time.data:
                            print(date)
                            if date not in output_dates:
                                update_netcdf(output_path, var_ds, date, areas_mask, veg_mask, args.temporaryfolder)
